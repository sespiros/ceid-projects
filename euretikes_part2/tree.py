...
def add_to_graph(t1, t2, s):
    ...
    if t2 is not None:
        ...
        f.write(str1 + '->' + str2 + ' [color = black]'+'[label="'+str(s) +'"]\n')
    else:
        f.write(str1 + '\n')
...
for i in iteration:
    ...
    for node in range(len(possible)):
        if possible[node] is None:
            fitness.append(1000)
        else:
            fitness.append(abs(5-possible[node][0]) + abs(5-possible[node][1]) + abs(0-possible[node][2])
                           + abs(i[best_ops[node][0]]-[10,0,0][best_ops[node][0]]))

    print(str(fullest) + "### Node  " + str(i) + "#########################")
